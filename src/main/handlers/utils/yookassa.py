...
async def create_yookassa_invoice(amount: str, description: str, token_count, user):
    ...
    data = {
        ...
        "receipt": {
            "customer": {"email": "aasemenov098.il@gmail.com"},
            "items": [
                {
        ...
        "capture": True,
        "confirmation": {"type": "redirect", "return_url": BOT_HOST},
    ...
    pay_dto = Pay(
        amount=amount, token_count=token_count, pay_id=response_data["id"], user=user, merchant=MerchantEnum.YOOKASSA
    )
    await pay_dto.asave()
