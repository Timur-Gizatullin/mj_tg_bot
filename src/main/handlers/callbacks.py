...
async def callbacks_variations(callback: types.CallbackQuery):
    ...
    if action == "V1":
        await send_variation_trigger(variation_index="1", queue=queue, user=telegram_user)
    elif action == "V2":
        await send_variation_trigger(variation_index="2", queue=queue, user=telegram_user)
    elif action == "V3":
        await send_variation_trigger(variation_index="3", queue=queue, user=telegram_user)
    elif action == "V4":
        await send_variation_trigger(variation_index="4", queue=queue, user=telegram_user)
...
async def callbacks_upsamples(callback: types.CallbackQuery):
    ...
    if action == "U1":
        await send_upsample_trigger(upsample_index="1", queue=queue, user=telegram_user)
    elif action == "U2":
        await send_upsample_trigger(upsample_index="2", queue=queue, user=telegram_user)
    elif action == "U3":
        await send_upsample_trigger(upsample_index="3", queue=queue, user=telegram_user)
    elif action == "U4":
        await send_upsample_trigger(upsample_index="4", queue=queue, user=telegram_user)
...
async def callback_vary(callback: types.CallbackQuery):
    ...
    if action == "strong":
        await send_vary_trigger(vary_type="high_variation", user=telegram_user, queue=queue)
    elif action == "subtle":
        await send_vary_trigger(vary_type="low_variation", user=telegram_user, queue=queue)
    elif action == "region":
        await send_vary_trigger(vary_type="variation", user=telegram_user, queue=queue)
